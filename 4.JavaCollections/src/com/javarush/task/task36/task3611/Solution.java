...
public class Solution {
    ...
    public Set<Integer> getAllFriendsAndPotentialFriends(int index, int deep) {
        TreeSet<Integer> result = getAllFriendsAndPotentialFriendsWithMe(index, deep);
        result.remove(index);
        return result;
    ...
    private TreeSet<Integer> getAllFriendsAndPotentialFriendsWithMe(int index, int deep) {
        ...
        if(deep > 0){
            for (int i = 0; i < humanRelationships.length; i++) {
                if((i< index) && (index < humanRelationships.length) && humanRelationships[index][i]){
                    friends.add(i);
                    friends.addAll(getAllFriendsAndPotentialFriends(i, deep - 1));
